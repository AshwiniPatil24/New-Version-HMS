...
using Ruby_Hospital.Properties;
...
{
    public partial class Assign_Lab_Reports : Form
    {
        public int PID;
        public string type;

        public Assign_Lab_Reports()
        {
            InitializeComponent();
        }
                ...
                if (this.type == "IPD")
                {
                    ...
                    foreach (DataGridViewRow row in dataGridView2.Rows)
                    {
                        ...
                        if (row.IsNewRow)
                            ...
                            string str4 = "";
                            string str5 = row.Cells["LabTest"].Value.ToString();
                            ...
                            SqlCommand command = new SqlCommand("insert into Ruby_Jamner123.Assign_Lab_Report (Patient_ID,Name,IPD_ID,OPD_ID,Test_Name,Test_Date,Sample_Date,Report_Date,Attached_File) \r\nvalues(@Patient_ID,@Name,@IPD_ID,@OPD_ID,@Test_Name,@Test_Date,@Sample_Date,@Report_Date,@Attached_File)", connection);
                ...
                if (this.type == "Only Test")
                {
                    ...
                    foreach (DataGridViewRow row2 in dataGridView2.Rows)
                    {
                        ...
                        if (row2.IsNewRow)
                            ...
                            string str14 = row2.Cells["LabTest"].Value.ToString();
                            ...
                            SqlCommand command2 = new SqlCommand("insert into Ruby_Jamner123.Assign_Lab_Report (Patient_ID,Name,IPD_ID,OPD_ID,Test_Name,Test_Date,Sample_Date,Report_Date,Attached_File) \r\nvalues(@Patient_ID,@Name,@IPD_ID,@OPD_ID,@Test_Name,@Test_Date,@Sample_Date,@Report_Date,@Attached_File)", connection);
            ...
            if ((e.RowIndex >= 0) && (e.ColumnIndex >= 0))
            {
                ...
                if (name.Equals("Name"))
                {
                    try
                    {
                        ...
                        SqlCommand selectCommand = new SqlCommand("SELECT * from Ruby_Jamner123.Assign_Lab_Report where Name = @name", connection);
                        ...
                        else
                        {
                            if (this.type == "IPD")
                            {
                                ...
                                SqlCommand command3 = new SqlCommand("select * from Ruby_Jamner123.PatientTestBilling_IPDnOnlyTest where IPDID = @IPDID and Balance = 0", connection);
                                ...
                                using (SqlDataReader reader = command3.ExecuteReader())
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        using (SqlCommand command13 = new SqlCommand("select LabTest,TestDate from Ruby_Jamner123.AssignIPDLabTest where IPDID = @IPDID", connection))
                                        {
                                        ...
                                        combinedDataTable.Columns.Add("LabTest", typeof(string));
                                        ...
                                        foreach (DataRow row1 in table2.Rows)
                                        {
                                            ...
                                            {
                                                combinedDataTable.Rows.Add(row1["Patient_ID"], row1["Name"], row1["IPD_ID"], row2["LabTest"], row2["TestDate"]);
                                            }
                                        }
                            ...
                            if(this.type == "Only Test")
                            {
                                ...
                                SqlCommand command3 = new SqlCommand("select * from Ruby_Jamner123.PatientTestBilling_IPDnOnlyTest where OPDID = @OPDID and Balance = 0", connection);
                                ...
                                using (SqlDataReader reader = command3.ExecuteReader())
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        using (SqlCommand command13 = new SqlCommand("select LabTest,TestDate from Ruby_Jamner123.AssignOnlyTest_Lab where OPDID = @OPDID", connection))
                                        {
                                        ...
                                        foreach (DataRow row1 in table2.Rows)
                                        {
                                            foreach (DataRow row2 in table3.Rows)
                                            {
                                                combinedDataTable.Rows.Add(row1["Patient_ID"], row1["Name"], row1["OPD_ID"], row2["LabTest"], row2["TestDate"]);
                                            }
                                        }
        ...
        private void Assign_Lab_Reports_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (this.type == "IPD")
                {
                    ...
                    foreach (DataGridViewRow row in dataGridView2.Rows)
                    {
                        ...
                        SqlCommand command = new SqlCommand("update Ruby_Jamner123.Assign_Lab_Report set Patient_ID = @Patient_ID,Name = @Name,IPD_ID = @IPD_ID,OPD_ID = @OPD_ID,Test_Name = @Test_Name,Test_Date = @Test_Date,Sample_Date = @Sample_Date,Report_Date = @Report_Date,Attached_File = @Attached_File where ID = @ID", connection);
                        command.Parameters.AddWithValue("@Patient_ID", str);
                        command.Parameters.AddWithValue("@Name", str2);
                        command.Parameters.AddWithValue("@IPD_ID", str3);
                        command.Parameters.AddWithValue("@OPD_ID", str4);
                ...
                if (this.type == "Only Test")
                {
                    ...
                    foreach (DataGridViewRow row2 in dataGridView2.Rows)
                    {
                        ...
                        SqlCommand command2 = new SqlCommand("update Ruby_Jamner123.Assign_Lab_Report set Patient_ID = @Patient_ID,Name = @Name,IPD_ID = @IPD_ID,OPD_ID = @OPD_ID,Test_Name = @Test_Name,Test_Date = @Test_Date,Sample_Date = @Sample_Date,Report_Date = @Report_Date,Attached_File = @Attached_File where ID = @ID", connection);
                        command2.Parameters.AddWithValue("@Patient_ID", str10);
                        command2.Parameters.AddWithValue("@Name", str11);
                        command2.Parameters.AddWithValue("@IPD_ID", str12);
                        command2.Parameters.AddWithValue("@OPD_ID", str13);
